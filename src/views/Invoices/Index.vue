                        ...
                        <tr v-for="(invoice, index) in invoices" :key="invoice.id">
                            ...
                            <td class="px-6 py-2 border-b border-gray-200 whitespace-nowrap text-center">
                                <div class="flex items-center justify-center">
                                    <router-link :to="{ path: 'invoices/show/' + invoice.id }"
                                    ...
                                    <router-link :to="{ path: 'refunds/invoices/' + invoice.id }"
...
const getInvoices = async (page) => {
    const response = await axios.get(`/invoices?page=${page}&per_page=${perPage.value}&search=${search.value}`);
...
const loadData = async (newPage) => {
    ...
    await getInvoices(currentPage.value);
}
