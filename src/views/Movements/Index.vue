<template>
    ...
    <div class="flex sjustify-between items-center flex-row ">
        ...
        <div class="flex w-26 flex-col sm:flex-row items-center">
            <router-link
                class="inline-flex items-center w-full sm:mb-0 mb-1 px-2 py-1 ml-2 bg-green-600 transition ease-in-out delay-75 hover:bg-green-700 text-white text-sm font-medium rounded-md"
                to="/movements/create">
                    ...
                    <tbody class="bg-white">
                        <tr v-for="(movement, index) in movements" :key="movement.id">
                            ...
                            <td class="px-6 py-2 border-b border-gray-200 whitespace-nowrap text-center">
                                <span :class="{
                                    'inline-flex px-2 capitalize text-xs font-semibold leading-5 text-red-800 bg-red-100 rounded-full': movement.tipoMovimiento === 0,
                                    'inline-flex px-2 capitalize text-xs font-semibold leading-5 text-green-800 bg-green-100 rounded-full': movement.tipoMovimiento === 1
                                }">
                                    {{ movement.tipoMovimiento === 0 ? 'Desabastecer' : 'Suministrar' }}
                                </span>
                            </td>
                            ...
                            <td class="px-6 py-2 border-b border-gray-200 whitespace-nowrap text-center">
                                <div class="flex items-center justify-center">
                                    <router-link :to="{ path: 'movements/show/' + movement.id }"
...
const getMovements = async (page) => {
    const response = await axios.get(`/movements?page=${page}&per_page=${perPage.value}&search=${search.value}`);
...
const loadData = async (newPage) => {
    ...
    await getMovements(currentPage.value);
}
