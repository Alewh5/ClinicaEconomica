                    ...
                    <tbody class="bg-white">
                        <tr v-for="(customer, index) in customers" :key="customer.id">
                            ...
                            <td class="px-6 py-2 border-b border-gray-200 whitespace-nowrap">
                                ...
                                <div class="text-sm leading-5 text-gray-500">
                                    {{ customer.municipio }} - {{ customer.departamento }}
                                </div>
                            </td>
                            <td class="px-6 py-2 border-b border-gray-200 whitespace-nowrap text-center">
                                <div class="flex items-center justify-center">
                                    <router-link :to="{ path: 'customers/edit/' + customer.id }"
                                    ...
                                    <a @click="deleteItem(customer.id, customer.NombreRazonSocial)"
...
const getCustomers = async (page) => {
    const response = await axios.get(`/customers?page=${page}&per_page=${perPage.value}&search=${search.value}`);
...
const deleteItem = async (id, name) => {
    confirmation(name, ('/customers/' + id), '', async () => {
        await loadData(1);
    });
}
