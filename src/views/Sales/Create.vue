<template>
    <div class="flex justify-between items-center">
        ...
        <div class="flex flex-col items-start w-26 sm:flex-row sm:items-center">
            <router-link
                class="inline-flex items-center text-xs mb-1 sm:mb-0 sm:mr-2 w-full px-2 h-8 bg-lime-500 transition ease-in-out delay-75 hover:bg-lime-700 text-white font-medium rounded-md"
                to="/sales">
    ...
    <div class="mt-6">
        <div class="px-4 py-4 bg-white shadow-md">
            <button @click="toggleForm()" class="flex items-center justify-between w-full outline-none">
                <hr class="border w-full mr-2">
            ...
            <form v-show="viewCustomer">
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 mt-2">
                    <div>
                        <label class="text-gray-700" for="tipoDocumento">Tipo de DNI <span
                                v-if="formErrorsCustomers['tipoDocumento']"
                        ...
                        <div class="flex mt-2">
                            ...
                            <select class="w-full bg-white rounded-r-md pl-2 text-base font-regular outline-0"
                                :class="{ 'border-red-500 border-2': formErrorsCustomers['tipoDocumento'] }"
                                id="tipoDocumento" v-model="formCustomer.tipoDocumento">
                    ...
                    <div>
                        <label class="text-gray-700" for="numeroDocumento">Documento <span
                                v-if="formErrorsCustomers['numeroDocumento']"
                                class="text-red-500 mt-1 text-sm">*</span></label>
                        <div class="flex mt-2">
                            <div
                                class="flex items-center justify-center rounded-tl-lg rounded-bl-lg border-y border-l relative border-gray-400 bg-white p-5">
                            ...
                            <input type="text"
                                ...
                                :class="{ 'border-red-500 border-2': formErrorsCustomers['numeroDocumento'] }"
                                id="numeroDocumento" v-model="formCustomer.numeroDocumento" autocomplete="off">
                        </div>
                    </div>

                    <div>
                        <label class="text-gray-700" for="NombreRazonSocial">Nombre / Razón Social <span
                                v-if="formErrorsCustomers['NombreRazonSocial']"
                                class="text-red-500 mt-1 text-sm">*</span></label>
                        <div class="flex mt-2">
                            <div
                                class="flex items-center justify-center rounded-tl-lg rounded-bl-lg border-y border-l relative border-gray-400 bg-white p-5">
                            ...
                            <input type="text"
                                class="w-full bg-white rounded-r-md pl-2 text-base font-regular outline-0"
                                :class="{ 'border-red-500 border-2': formErrorsCustomers['NombreRazonSocial'] }"
                                id="NombreRazonSocial" v-model="formCustomer.NombreRazonSocial" autocomplete="off">
                        </div>
                    </div>

                    <div>
                        <label class="text-gray-700" for="email">Email <span v-if="formErrorsCustomers['email']"
                                class="text-red-500 mt-1 text-sm">*</span></label>
                        <div class="flex mt-2">
                            <div
                                class="flex items-center justify-center rounded-tl-lg rounded-bl-lg border-y border-l relative border-gray-400 bg-white p-5">
                            ...
                            <input type="email"
                                class="w-full bg-white rounded-r-md pl-2 text-base font-regular outline-0"
                                :class="{ 'border-red-500 border-2': formErrorsCustomers['email'] }" id="email"
                                v-model="formCustomer.email" autocomplete="off">
                        </div>
                    </div>

                    <div>
                        <label class="text-gray-700" for="telefono">Teléfono <span
                                v-if="formErrorsCustomers['telefono']"
                                class="text-red-500 mt-1 text-sm">*</span></label>
                        <div class="flex mt-2">
                            <div
                                class="flex items-center justify-center rounded-tl-lg rounded-bl-lg border-y border-l relative border-gray-400 bg-white p-5">
                            ...
                            <input type="text"
                                class="w-full bg-white rounded-r-md pl-2 text-base font-regular outline-0"
                                :class="{ 'border-red-500 border-2': formErrorsCustomers['telefono'] }" id="telefono"
                                v-model="formCustomer.telefono" autocomplete="off">
                        </div>
                    </div>
                </div>

                <button type="button" @click="saveCustomer()"
                    class="flex items-center text-xs mt-4 w-24 px-2 h-8 bg-blue-600 transition ease-in-out delay-75 hover:bg-blue-700 text-white font-medium rounded-md">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="bi bi-floppy-fill h-4 w-4 mr-2"
                        viewBox="0 0 16 16">
                        <path
                            d="M0 1.5A1.5 1.5 0 0 1 1.5 0H3v5.5A1.5 1.5 0 0 0 4.5 7h7A1.5 1.5 0 0 0 13 5.5V0h.086a1.5 1.5 0 0 1 1.06.44l1.415 1.414A1.5 1.5 0 0 1 16 2.914V14.5a1.5 1.5 0 0 1-1.5 1.5H14v-5.5A1.5 1.5 0 0 0 12.5 9h-9A1.5 1.5 0 0 0 2 10.5V16h-.5A1.5 1.5 0 0 1 0 14.5z" />
        ...
        <div class="px-4 pb-8 pt-5 bg-white shadow-md">
            <form>
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
                    ...
                    <div v-if="!form.customer_id" class="relative">
                        <label class="text-gray-700" for="searchCustomer">Buscar Clientes <span
                                v-if="formErrors['customer_id']" class="text-red-500 mt-1 text-sm">*</span></label>
                        <div class="flex mt-2">
                            <div
                                class="flex items-center justify-center rounded-tl-lg rounded-bl-lg border-y border-l relative border-gray-400 bg-white p-5">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor"
                            ...
                            <input type="text"
                                class="w-full bg-white rounded-r-md pl-2 text-base font-regular outline-0"
                                :class="{ 'border-red-500 border-2': formErrors['customer_id'] }" id="searchCustomer"
                                v-model="searchCustomer" autocomplete="off" @input="searchCustomers">
                        </div>
                        <!-- Mostrar clientes filtrados -->
                        <div v-if="searchCustomer.length > 0"
                            class="mt-4 absolute top-16 bg-white z-10 border rounded border-gray-300 w-full overflow-y-auto">
                            <ul>
                                <li v-for="customer in customerFilters.slice(0, 5)" :key="customer.id"
                                    class="text-gray-700 border-b border-gray-300 p-2 flex items-center justify-between">
                                    <span class="font-medium">
                                        {{ customer.NombreRazonSocial }}</span>
                    ...
                    <div v-if="form.customer_id">
                        <label class="text-gray-700" for="customer_id">Cliente <span v-if="formErrors['customer_id']"
                                class="text-red-500 mt-1 text-sm">*</span></label>
                        <div class="flex mt-2">
                            <div
                                class="flex items-center justify-center rounded-tl-lg rounded-bl-lg border-y border-l relative border-gray-400 bg-white p-5">
                            ...
                            <input type="text"
                                class="w-full bg-white rounded-r-md pl-2 text-base font-regular outline-0"
                                :class="{ 'border-red-500 border-2': formErrors['customer_id'] }" id="customer_id"
                                v-model="cliente.NombreRazonSocial" autocomplete="off">
                        </div>
                    </div>
                    <!-- CIERRE -->
                    ...
                    <div>
                        <label class="text-gray-700" for="metodoPago">Metodo de Pago <span
                                v-if="formErrors['metodoPago']" class="text-red-500 mt-1 text-sm">*</span></label>
                        <div class="flex mt-2">
                            <div
                                class="flex items-center justify-center rounded-tl-lg rounded-bl-lg border-y border-l relative border-gray-400 bg-white p-5">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor"
                                ...
                                <tr>
                                    ...
                                    <td class="px-1 py-1 border-b border-gray-200 text-center ">
                                        <vue-number type="text"
                                            class=" bg-white w-full h-0 py-3 rounded pl-2 text-base font-regular outline-0"
                                            style="font-size: 0.7rem"
                                            :class="{ 'border-red-500 border-2': formErrors[''] }" id=""
                                            v-model="form.descuento_global" autocomplete="off" v-bind="{ prefix: '% ' }"
                                            @change="calcularTotales()"></vue-number>
                                    </td>
                                    <td class="px-1 py-1 border-b border-gray-200 text-center ">
                                        <vue-number type="text"
                                            class=" bg-white w-full h-0 py-3 rounded pl-2 text-base font-regular outline-0"
                                            style="font-size: 0.7rem"
                                            :class="{ 'border-red-500 border-2': formErrors[''] }" id=""
                                            v-model="form.valor_descuentoGlobal" autocomplete="off"
